...
export function Platinum() {
  ...
  return (
    <div>
          ...
          return (
            <article
              ...
              className={`flex flex-col rounded-2xl border p-5 ${
                featured ? 'border-accent/60 bg-accent/5' : 'border-border/70 bg-card'
              }`}
            >
              <div className="flex items-start justify-between gap-2">
                ...
                {current && (
                  <span className="shrink-0 rounded-full bg-accent px-2 py-0.5 text-[11px] font-semibold text-on-accent">
                    aktif
                  </span>
                )}
              </div>
      ...
      <Link
        to="/profil"
        className="mt-4 block rounded-xl border border-border/70 px-4 py-2.5 text-center text-sm font-medium transition-colors duration-200 hover:bg-muted"
      >
        Profile dön
      </Link>
    </div>
